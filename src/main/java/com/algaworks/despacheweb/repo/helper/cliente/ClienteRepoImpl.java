...
public class ClienteRepoImpl implements ClienteQueries{
	...
	public Page<Cliente> filtrar(ClienteFilter filtro, Pageable pageable) {
		...
		criteria.createAlias("naturalidade", "n", JoinType.LEFT_OUTER_JOIN);
	...
	private void adicionaFiltro(ClienteFilter filtro, Criteria criteria) {
		if(filtro != null){
			...
			if (!StringUtils.isEmpty(filtro.getCpfOuCnpj())) {
				criteria.add(Restrictions.ilike("cpfOuCnpj", filtro.getCpfOuCnpj(), MatchMode.EXACT));
			}
			
			if (isEstadoNaturalidadePresente(filtro)) {
				criteria.add(Restrictions.eq("n.estado", filtro.getEstado()));
			}
	...
	private boolean isEstadoNaturalidadePresente(ClienteFilter filtro) {
	...
	public Cliente retornaCliente(Cliente cliente) {
		...
		criteria.createAlias("c.estado", "e", JoinType.LEFT_OUTER_JOIN);
		criteria.createAlias("naturalidade", "n", JoinType.LEFT_OUTER_JOIN);
		criteria.createAlias("n.estado", "ne", JoinType.LEFT_OUTER_JOIN);
